...
The single most important honesty item: our **headline claim is mechanism-demonstrated but not yet outcome-measured.**

- **Headline claim — "agents carry forward what worked → measurable lift."** The *mechanism* exists and runs (the turn-loop reliability pillars in `scripts/aios_turn_loop.py`, the append-only run record / AkashicRecord). What is **not** yet done is the *outcome* measurement: a controlled ledger-on vs. ledger-off A/B showing the lift is real. That A/B is being run in parallel — see `docs/AIOS_HEADLINE_AB_RESULTS.md` (forthcoming; owned by another workstream). Until it lands, treat the headline as a demonstrated mechanism, not a measured result.
